...
async function refreshSession() {
  let localSession = localStorage.getItem('rent_a_screen_session_id');
  if (localSession) {
    ...
    if (data.status === 200 ) {
      loggedIn.value = true;
      user.value = { username: (data.user as any).username};
    }
  }
}
...
async function clear() {
  ...
  localStorage.removeItem('rent_a_screen_session_id');
  session.value = '';
...
<template>
  <header
    class="flex flex-col lg:flex-row justify-between items-center bg-zinc-900 rounded-2xl pl-4 pr-4 pb-1 fixed w-11/12 inset-x-0 mx-auto top-8">
      ...
      <ul class="flex flex-col lg:flex-row lg:gap-5">
        ...
        <li>
          <NuxtLink href="/purchase"
            class="flex lg:px-3 py-2 border rounded bg-white-important border-gray-100 text-gray-900 pointer"
            :class="{ 'tabSelected': location == '/purchase' }">Rent a Screen</NuxtLink>
        </li>
      </ul>
